...
void RecHitTreeProducer::analyze(const edm::Event& ev, const edm::EventSetup& iSetup) {
  hfRecHit.n = 0;
  hbheRecHit.n = 0;
  ebRecHit.n = 0;
  eeRecHit.n = 0;
  ...
  if (doHF_ && !doEbyEonly_) {
    ...
    for (auto const& hit : *hfHits) {
      if (getEt(hit.id(), hit.energy(), geo) < hfPtMin_)
        continue;
      ...
      if (id.ieta() > 0) {
        if (hit.energy() > hfShortThreshold_ && id.depth() != 1)
          nHFshortPlus++;
        if (hit.energy() > hfLongThreshold_ && id.depth() == 1)
          nHFlongPlus++;
      } else {
        if (hit.energy() > hfShortThreshold_ && id.depth() != 1)
          nHFshortMinus++;
  ...
  if (doHBHE_ && !doEbyEonly_) {
    ...
    for (auto const& hit : *hbheHits) {
      if (getEt(hit.id(), hit.energy(), geo) < hbhePtMin_)
        continue;
  ...
  if (doEcal_ && !doEbyEonly_) {
    ...
    for (unsigned int i = 0; i < ebHits->size(); ++i) {
      ...
      if (getEt(hit.id(), hit.energy(), geo) < ebPtMin_)
        continue;
    ...
    for (unsigned int i = 0; i < eeHits->size(); ++i) {
      ...
      if (getEt(hit.id(), hit.energy(), geo) < eePtMin_)
        continue;
  ...
  if (!doEbyEonly_ && doTowers_) {
    ...
    for (auto const& hit : *towers) {
      ...
      if (hit.ieta() > 29 && hit.energy() > hfTowerThreshold_)
        nHFtowerPlus++;
      if (hit.ieta() < -29 && hit.energy() > hfTowerThreshold_)
        nHFtowerMinus++;
...
void RecHitTreeProducer::beginJob() {
  if (!doEbyEonly_) {
    if (doHBHE_) {
...
void RecHitTreeProducer::endJob() {}
