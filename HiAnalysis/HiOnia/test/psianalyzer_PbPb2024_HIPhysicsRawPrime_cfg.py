...
options.outputFile = 'Oniatree_PsiMassRegion_PbPb2024_HIPhysicsRawPrime.root'
...
triggerList    = {
		...
		'DoubleMuonTrigger' : cms.vstring(
                ...
                'SingleMuonTrigger' : cms.vstring(
                        "HLT_HIL1SingleMu0_Open_v",#3
                        "HLT_HIL1SingleMu0_v",#4
                        "HLT_HIL1SingleMu0_Centrality40to100_v",#5
                        "HLT_HIL1SingleMu0_Centrality30to100_v",#6
                        "HLT_HIL1SingleMuOpen_Centrality30to100_v",#7
                        "HLT_HIMinimumBiasHF1AND_v", #8
                        "HLT_HIMinimumBiasHF1ANDZDC1nOR_v", #9
			)
    }
...
oniaTreeAnalyzer(process,
                 ...
                 muonSelection=muonSelection, L1Stage=2, isMC=isMC, pdgID=pdgId, outputFileName=options.outputFile, doTrimu=doTrimuons, OnlySingleMuons=False
)

process.onia2MuMuPatGlbGlb.dimuonSelection       = cms.string("mass > 2.5 && mass < 4.0 && abs(daughter('muon1').innerTrack.dz - daughter('muon2').innerTrack.dz) < 20")
process.onia2MuMuPatGlbGlb.lowerPuritySelection  = cms.string("pt > 1.0 && abs(eta) < 2.41 && isTrackerMuon && innerTrack.hitPattern.trackerLayersWithMeasurement > 5 && innerTrack.hitPattern.pixelLayersWithMeasurement > 0")
...
if applyEventSel:
  ...
  # Muon filtering
  SuperLooseMuonCut = "isTrackerMuon && pt > 1.0 && abs(eta) < 2.41 && innerTrack.hitPattern.trackerLayersWithMeasurement > 5 && innerTrack.hitPattern.pixelLayersWithMeasurement > 0"
  ...
  process.dimuonSelection = cms.EDProducer("CandViewShallowCloneCombiner",
                                    ...
                                    cut = cms.string("mass > 2.4 && mass < 4.0"),
  ...
  process.oniaTreeAna.replace(process.patMuonSequence,process.muonSelector * process.atLeastTwoMuons * process.dimuonSelection * process.atLeastOneDimuon * process.phfCoincFilterPF2Th4 * process.primaryVertexFilter * process.clusterCompatibilityFilter * process.patMuonSequence )
