...
TriggerObjectAnalyzer::TriggerObjectAnalyzer(const edm::ParameterSet& ps)
{
  ...
  triggerObjectsTag_ = consumes<std::vector<pat::TriggerObjectStandAlone>>(ps.getParameter<edm::InputTag>("triggerObjects"));
...
TriggerObjectAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  if(hltConfig_.size() > 0){
	...
	// do not fill trigger object if the event is not accepted.
      ...
      for (pat::TriggerObjectStandAlone tObj : *triggerObjectsHandle_) { // note: not "const &" since we want to call unpackPathNames
        ...
        if ( tObj.hasPathName(triggerNames_[itrig], requireLastFilterAccept, requireL3FilterAccept) ) {
          ...
          if(verbose_){
            std::cout << "verbose : "
	              << tObj.pdgId() << " " << tObj.pt() << " " << tObj.et() << " " << tObj.eta() << " " << tObj.phi() << " " << tObj.mass()
	              << std::endl;
	  }
	  id[itrig].push_back(tObj.pdgId());  // TriggerObjectStandAlone::pdgId() does not necessarily correspond to what is in AOD counterpart, but its the closest by comparing descriptions. Also, this branch is rather useless, could be removed later on
...
TriggerObjectAnalyzer::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{
  ...
  if (hltConfig_.init(iRun,iSetup,processName_,changed)) {
	...
	if (inMenu==triggerInMenu.end()) {
            std::cout << "<HLT Object Analyzer> Warning! Trigger " << triggerNames_[itrig] << " not found in HLTMenu. Skipping..." << std::endl;
        }
      }
      if(verbose_){
