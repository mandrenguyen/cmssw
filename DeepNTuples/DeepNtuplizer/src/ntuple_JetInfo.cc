...
void ntuple_JetInfo::initBranches(TTree* tree){
    ...
    addBranch(tree,"weight"    ,&weight_    ,"weight/F"    );


    // truth labels
    addBranch(tree,"gen_pt"    ,&gen_pt_    ,"gen_pt_/F"    );
    addBranch(tree,"Delta_gen_pt"    ,&Delta_gen_pt_,"Delta_gen_pt_/F"    );
    addBranch(tree,"hflav",&hflav_, "hflav_/i");
    addBranch(tree,"pflav",&pflav_, "pflav_/i");
...
//    addBranch(tree,"isPhysTau",&isPhysTau_, "isPhysTau_/i");
    ...
    addBranch(tree,"jet_phf", &jet_phf_);
    addBranch(tree,"jet_elf", &jet_elf_);
    addBranch(tree,"jet_muf", &jet_muf_);
    addBranch(tree,"jet_ncand", &jet_ncand_);
...
void ntuple_JetInfo::readEvent(const edm::Event& iEvent){
    ...
    weight_ = genEventInfoHandle->weight();
...
bool ntuple_JetInfo::fillBranches(const pat::Jet & unsubJet, const pat::Jet & jet, const size_t& unsubJetidx, const size_t& jetidx, const edm::View<pat::Jet> * unsubColl, const edm::View<pat::Jet> * coll){
    ...
    float pthat = genEventInfoHandle->qScale();
    ...
    const auto jetRef = reco::CandidatePtr(unsubColl->ptrs().at( unsubJetidx));
    ...
    if(jet.genJet()!=NULL){
      hflav_ = abs(unsubJet.hadronFlavour());
      pflav_ = abs(unsubJet.partonFlavour());

      //switch(deep_ntuples::jet_flavour(jet, gToBB, gToCC, neutrinosLepB, neutrinosLepB_C, alltaus_)) {
      switch(deep_ntuples::jet_flavour(unsubJet, gToBB, gToCC, neutrinosLepB, neutrinosLepB_C, alltaus_)) {
    ...
    if(jet.genJet()!=NULL){
      ...
      switch(deep_ntuples::jet_flavour(unsubJet, gToBB, gToCC, neutrinosLepB, neutrinosLepB_C, alltaus_,true)) {
    ...
    if(jet_pt_ < 0.35 * pthat ) returnval = false;
	...
	jet_chf_ = CHF;
	jet_nhf_ = NHF;
	jet_phf_ = jet.photonEnergyFraction();
	jet_elf_ = jet.electronEnergyFraction();
	jet_muf_ = jet.muonEnergyFraction();
	jet_ncand_ = jet.chargedHadronMultiplicity()+jet.neutralHadronMultiplicity()+jet.electronMultiplicity()+jet.photonMultiplicity()+jet.muonMultiplicity();
