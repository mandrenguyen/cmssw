...
void ntuple_SV::initBranches(TTree* tree){
    ...
    addBranch(tree,(prefix_+"sv_energy").c_str()     ,&sv_energy_     ,(prefix_+"sv_energy_["+prefix_+"sv_num_]/F").c_str());
    addBranch(tree,(prefix_+"sv_deta").c_str()     ,&sv_deta_     ,(prefix_+"sv_deta_["+prefix_+"sv_num_]/F").c_str());
    addBranch(tree,(prefix_+"sv_dphi").c_str()     ,&sv_dphi_     ,(prefix_+"sv_dphi_["+prefix_+"sv_num_]/F").c_str());
...
bool ntuple_SV::fillBranches(const pat::Jet & unsubJet, const pat::Jet & jet, const size_t& unsubJetidx, const size_t& jetidx, const  edm::View<pat::Jet> * unsubColl, const  edm::View<pat::Jet> * coll){
    ...
    for (const reco::VertexCompositePtrCandidate &sv : cpvtx) {
      //std::cout<<"scond"<<std::endl;
        ...
        if((int)max_sv>sv_num_){
            ...
            sv_deta_[sv_num_]       = catchInfsAndBound(fabs(sv.eta()-jet.eta())-0.5,0,-2,0);
            sv_dphi_[sv_num_]       = catchInfsAndBound(fabs(reco::deltaPhi(sv.phi(),jet.phi()))-0.5,0,-2,0);
