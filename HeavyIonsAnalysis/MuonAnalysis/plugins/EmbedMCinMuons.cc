...
void pat::EmbedMCinMuons::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (size_t i = 0; i < muons->size(); ++i) {
    const auto genRef = matches.get(muons.id(), i);
...
using namespace pat;
// HiAnalysis/HiOnia provides the EmbedMCinMuons registration needed by the
// combined MC Onia sequence.  Keep only one registration in this CMSSW area.
