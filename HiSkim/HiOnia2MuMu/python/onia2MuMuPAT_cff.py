...
def changeToMiniAOD(process):
    ...
    if hasattr(process, "patMuonsWithTrigger"):
        ...
        # Reuse an unpacker already configured by HiForest.  Loading another
        # autogenerated CFI with the same label would try to replace a module
        # that is already held by the HiForest track sequence.
        if not hasattr(process, "unpackedTracksAndVertices"):
        ...
        if not hasattr(process, "unpackedMuons"):
