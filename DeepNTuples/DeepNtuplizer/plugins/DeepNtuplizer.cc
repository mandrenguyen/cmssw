...
private:
    ...
    edm::EDGetTokenT<edm::View<pat::Jet> >      unsubJetToken_;
...
DeepNtuplizer::DeepNtuplizer(const edm::ParameterSet& iConfig):
  ...
  unsubJetToken_(consumes<edm::View<pat::Jet> >(iConfig.getParameter<edm::InputTag>("unsubJets"))),
...
DeepNtuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    edm::Handle<edm::View<pat::Jet> > unsubJets;
    iEvent.getByToken(unsubJetToken_, unsubJets);


    //for( auto& unsubJet : unsubJets) std::cout<<" unsubJet.pt() "<<unsubJet.pt()<<std::endl;
    ...
    std::vector<size_t> unsubIndices(unsubJets->size());

    for(size_t i=0;i<jets->size();i++)
        indices.at(i)=i;
    for(size_t k=0;k<unsubJets->size();k++)
        unsubIndices.at(k)=k;
	...
	float matchDr2 = 999;
	//float matchPt = -999;
	size_t closestUnsubJetIdx;
	float jetPhi = jet.phi();
	float jetEta = jet.eta();
	//std::cout<<" jet pt "<<jet.pt()<<" jetPhi "<<jetPhi<<" jetEta "<<jetEta<<std::endl;

	edm::View<pat::Jet>::const_iterator unsubJetIter;
	for(size_t k=0;k<unsubIndices.size();k++){


	  size_t unsubJetidx=unsubIndices.at(k);
	  unsubJetIter = unsubJets->begin()+unsubJetidx;
	  const pat::Jet& unsubJet = *unsubJetIter;
	  //std::cout<<" k "<<k<<" unsubJetidx "<<unsubJetidx<<std::endl;
	  
	//for (const pat::Jet& unsubJet : *unsubJets) {
	  float unsubJetPhi = unsubJet.phi();
	  float unsubJetEta = unsubJet.eta();
	  //std::cout<<" unsubJetPhi "<<unsubJetPhi<<" unsubJetEta "<<unsubJetEta<<std::endl;
	  float dphi = acos(cos(jetPhi - unsubJetPhi));
	  float deta = jetEta - unsubJetEta;
	  float dr2 = dphi*dphi + deta*deta;
	  if(dr2 < matchDr2) {
	    matchDr2 = dr2;
	    //matchPt = unsubJet.pt();
	    closestUnsubJetIdx = unsubJetidx;
	  }
	}

        unsubJetIter = unsubJets->begin()+closestUnsubJetIdx;
        const pat::Jet& unsubJet = *unsubJetIter;
	...
	size_t idx = 0;
	  ...
	  if(! m->fillBranches(unsubJet, jet, closestUnsubJetIdx, jetidx, unsubJets.product(), jets.product())){
	    writejet=false;
