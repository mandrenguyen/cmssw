...
PFCandCompositeProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(nComp>0){
	...
	// only selection is pt > 3 GeV for now
	if( seld0Cand(cand)){	 
	  bool isDup = false;
	  ...
	  double candE = sqrt(cand.p()*cand.p() + d0mass2);
	...
	if( selJpsiCand(cand) && selMuonCand(cand,"muon1") && selMuonCand(cand,"muon2") ){
	  ...
	  double candE = sqrt(cand.p()*cand.p() + jPsimass2);
	...
	for(std::vector<CompositeCandidate>::const_iterator it=selComposites.begin();
	  ...
	  // TODO:  check that this precision makes sense
	  double eps = 0.005;                                                                                                                                                   
...
void PFCandCompositeProducer::beginStream(edm::StreamID) {}

// ------------ method called once each stream after processing all runs, lumis and events  ------------                                                                                                             
void PFCandCompositeProducer::endStream() {}

  //PFCandCompositeProducer::seld0Cand(const CompositeCandidate d0Cand) {
 bool
  PFCandCompositeProducer::seld0Cand(const CompositeCandidate d0Cand) const {
