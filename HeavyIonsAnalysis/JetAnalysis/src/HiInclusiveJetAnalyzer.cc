...
HiInclusiveJetAnalyzer::HiInclusiveJetAnalyzer(const edm::ParameterSet& iConfig) {
  ...
  if (doSubJets_)
    subjetTag_ = consumes<reco::JetView>(iConfig.getUntrackedParameter<InputTag>("subjetTag"));
...
void HiInclusiveJetAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup) {
  ...
  if (doSubJets_)
    iEvent.getByToken(subjetTag_, subjets_);
  if (doGenSym_) {
...
void HiInclusiveJetAnalyzer::analyzeSubjets(const reco::Jet& jet) {
  ...
  const reco::Jet* groomedJet = nullptr;
  double minDr = rParam;
  for (const auto& candidate : *subjets_) {
    const double dr = deltaR(jet, candidate);
    if (dr < minDr) {
      minDr = dr;
      groomedJet = &candidate;
    }
  }
  if (groomedJet != nullptr && groomedJet->numberOfDaughters() > 0) {
    for (unsigned k = 0; k < groomedJet->numberOfDaughters(); ++k) {
      const reco::Candidate& dp = *groomedJet->daughter(k);
